...
export default function FacebookStoriesPreview({ headline, cta, imageUrl, logoUrl, brandName }) {
  return (
    <PreviewFrame width={375} style={{ height: 667 }}>
      <div style={{ fontFamily: "-apple-system, Helvetica, sans-serif" }} className="relative w-full h-[667px] bg-black overflow-hidden">
        ...
        <div className="absolute bottom-24 left-4 right-4 text-white">
          <h2 className="text-xl font-bold drop-shadow-lg">{headline}</h2>
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-4 text-center">
          <div className="rounded-full border border-white/50 bg-white/10 backdrop-blur py-3 text-white text-sm font-semibold">
            Swipe Up ↑ · {cta}
          </div>
          <button type="button" className="mt-2 text-xs text-white/70 underline">See More</button>
        </div>
      </div>
    </PreviewFrame>
  );
