...
export default function GoogleAdsConnectPanel({
  ...
  const importFromConnectedGoogleAds = useCallback(async () => {
    ...
    const user = getFirebaseClientAuth().currentUser;
    if (!user) {
      setError("Sign in to Adigator IQ first. Import saves campaigns under your Adigator IQ account.");
      return;
    }
    ...
    try {
      const token = await user.getIdToken();
      if (!token) throw new Error("Unable to verify Adigator IQ session. Please sign in again.");
      ...
      setImportSuccess(`Imported ${sourceLabel} campaign and saved it in Adigator IQ.`);
    } catch (err) {
          ...
          <div className="rounded-2xl border border-white/15 bg-black/10 p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                className="studio-btn-primary studio-focus-ring inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
              >
            ...
            <div className="mt-4 space-y-3 rounded-xl border border-cyan-400/20 bg-cyan-500/8 p-3">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-200">Import Campaign Into Adigator IQ</p>
              ...
              >
                ...
                {importLoading ? "Importing..." : "Import & Save in Adigator IQ"}
              </button>
