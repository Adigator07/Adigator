...
export function useMetaCreativeAnalysis(selectedSource) {
  ...
  const analyze = useCallback(async (source) => {
    ...
    try {
      ...
      const heuristicElements = detectElementsFromImageData(imageData, canvas.width, canvas.height).map((el) => ({
        ...el,
        x: el.x * scaleX,
        y: el.y * scaleY,
      ...
      try {
        const response = await fetch("/api/meta-creative-analysis", {
          method: "POST",
        ...
        if (payload?.ok && Array.isArray(payload.elements) && payload.elements.length) {
          const visionElements = normalizeApiElements(payload.elements, width, height);
        ...
        // Vision enhancement is optional; heuristics still power the UI.
      }
