...
import {
  detectGoogleElementsFromImageData,
  mergeElementDetections,
  normalizeGoogleApiElements,
...
export function useGoogleCreativeAnalysis(selectedSource) {
  ...
  const analyze = useCallback(async (source) => {
    ...
    try {
      ...
      const heuristicElements = detectGoogleElementsFromImageData(imageData, canvas.width, canvas.height).map((el) => ({
        ...el,
        x: el.x * scaleX,
        y: el.y * scaleY,
      ...
      try {
        const response = await fetch("/api/google-creative-analysis", {
          method: "POST",
        ...
        if (payload?.ok && Array.isArray(payload.elements) && payload.elements.length) {
          const visionElements = normalizeGoogleApiElements(payload.elements, width, height);
        ...
        // Vision enhancement is optional.
      }
