...
interface DetectedElement {
  type: "text" | "logo" | "face" | "cta" | "product" | "brand";
  label: string;
  x: number;
  y: number;
  width: number;
...
export async function POST(request: NextRequest) {
  try {
      ...
      messages: [
        {
          role: "system",
          content: [
            "Detect advertising elements for programmatic IAB banner safe-zone analysis.",
            "Return JSON: { elements: [{ type, label, x, y, width, height, confidence }] }.",
            "Normalized 0-1 coordinates. Detect logos, headlines, body text, CTAs, product images, faces, brand elements.",
            "Up to 12 elements with precise bounding boxes.",
          ].join(" "),
        },
        {
          role: "user",
          content: [
            ...
            { type: "text", text: "Detect all important ad elements with bounding boxes for IAB display analysis." },
          ],
        },
      ],
    });
    ...
    const message = error instanceof Error ? error.message : "Programmatic creative analysis failed";
    console.error("[programmatic-creative-analysis]", message);
