...
interface DetectedElement {
  type: "text" | "logo" | "face" | "cta" | "product";
  label: string;
  x: number;
  y: number;
  width: number;
  ...
  return raw
    .map((item) => {
      ...
      const normalizedType = type.includes("logo")
        ? "logo"
        : type.includes("product") || type.includes("subject") || type.includes("item")
          ? "product"
...
export async function POST(request: NextRequest) {
  try {
      ...
      messages: [
        {
          role: "system",
          content: [
            "You detect important advertising creative elements for Google Ads safe-zone analysis.",
            ...
            "Detect: headlines/text (OCR regions), logos, faces, products/subjects, CTA buttons.",
            "Include up to 12 elements. Be precise with bounding boxes.",
          ].join(" "),
        },
        {
          role: "user",
          content: [
            ...
            {
              type: "text",
              text: "Detect headlines, body text, logos, faces, products/subjects, and CTA buttons with bounding boxes for Google Ads placements.",
            },
          ],
        },
      ],
    ...
    const message = error instanceof Error ? error.message : "Google creative analysis failed";
    console.error("[google-creative-analysis]", message);
