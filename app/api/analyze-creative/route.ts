...
import {
  ...
  CATEGORY_TO_SUGGESTED_VERTICAL,
...
function resolveVerticalIsAligned(
  ...
  const suggestedVertical = CATEGORY_TO_SUGGESTED_VERTICAL[detected] || detected;
  ...
  if (aiAnalysis?.vertical_match === true) {
    return true;
  }
  ...
  if (detected !== selectedVertical) {
    if (areRelatedVerticals(detected, selectedVertical) && fitScore >= 45) {
      return true;
    }
    return false;
  }
...
export type AnalyzeCreativeRunInput = {
  file: File;
  goal: string;
...
export async function runAnalyzeCreative(input: AnalyzeCreativeRunInput): Promise<Record<string, unknown>> {
  ...
  const goal = normalizeGoal(input.goal || "awareness");
  const goalProfile = GOAL_INTELLIGENCE_PROFILE[goal];
  const audienceStage = normalizeAudienceStage(input.audienceStage || "cold");
  const vertical = normalizeVertical(input.vertical || "technology");
  const platform = normalizePlatform(input.platform || "programmatic");
  ...
  if (!file.type.startsWith("image/")) {
    throw new Error("Invalid file type. Upload an image.");
  }
  ...
  let aiAnalysis = applyPlatformToneGuard(
    platform,
    extractionResult.aiAnalysis as AIAnalysisOutput | null,
    goal,
    vertical,
  ) as AIAnalysisOutput | null;
  ...
  const metaFeedIntelligence = buildMetaFeedIntelligence({
    ...
    aiAnalysis = applyPlatformToneGuard(platform, aiAnalysis, goal, vertical) as AIAnalysisOutput | null;
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await runAnalyzeCreative({
      file,
      goal: String(formData.get("goal") || "awareness"),
      audienceStage: String(formData.get("audience_stage") || "cold"),
