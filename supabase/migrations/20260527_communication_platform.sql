-- Adigator Communication Platform — Phase 1 schema
-- Hybrid chat + email creative review system with activity tracking.
-- Safe to run idempotently (IF NOT EXISTS / DROP POLICY IF EXISTS).
...
-- ── messages ─────────────────────────────────────────────────────────────────
-- metadata example: { "delivery_status": "read", "reply_to": "uuid" }
...
-- event_data example: { "file_name": "banner.jpg", "duration_seconds": 42, "device": "desktop" }
...
-- ── message_read_receipts (dedupe message_opened events) ─────────────────────
...
-- ── Helper: is conversation participant ──────────────────────────────────────
...
-- attachments (via message participant check)
...
-- ── Storage bucket for communication files ───────────────────────────────────
