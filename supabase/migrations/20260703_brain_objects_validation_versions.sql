-- Brain objects and immutable validation versioning (campaign-level).
-- Brains are append-only; validation_versions and validation_brains are insert-only.
...
-- ── validation_brains (immutable alignment snapshots) ──────────────────────────
...
-- ── validation_versions (immutable version chain) ────────────────────────────────
...
-- Link programmatic campaigns to latest validation version (optional, lazy backfill).
do $$
begin
  if exists (
...
create policy "campaign_brains_select_own"
on public.campaign_brains for select
using (auth.uid() = user_id);

drop policy if exists "campaign_brains_insert_own" on public.campaign_brains;
create policy "campaign_brains_insert_own"
on public.campaign_brains for insert
...
create policy "landing_page_brains_select_own"
on public.landing_page_brains for select
using (auth.uid() = user_id);

drop policy if exists "landing_page_brains_insert_own" on public.landing_page_brains;
create policy "landing_page_brains_insert_own"
on public.landing_page_brains for insert
with check (auth.uid() = user_id);

-- ── RLS: validation_brains (insert + select only) ──────────────────────────────
...
create policy "validation_brains_select_own"
on public.validation_brains for select
using (auth.uid() = user_id);

drop policy if exists "validation_brains_insert_own" on public.validation_brains;
create policy "validation_brains_insert_own"
on public.validation_brains for insert
with check (auth.uid() = user_id);

-- ── RLS: validation_versions (insert + select only) ────────────────────────────
...
create policy "validation_versions_select_own"
on public.validation_versions for select
using (auth.uid() = user_id);

drop policy if exists "validation_versions_insert_own" on public.validation_versions;
create policy "validation_versions_insert_own"
on public.validation_versions for insert
with check (auth.uid() = user_id);

-- ── Version 1 backfill marker ──────────────────────────────────────────────────
-- Existing programmatic_campaigns snapshots become Version 1 lazily when the
-- Validation Orchestrator first runs for a campaign. No destructive backfill here.
